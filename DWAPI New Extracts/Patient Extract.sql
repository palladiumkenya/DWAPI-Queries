select CASE WHEN prg.program = 'TB' THEN prg.status ELSE null end                     AS StatusAtTBClinic,
       ...
       CAST(GREATEST(coalesce(max(hiv.visit_date), date('1000-01-01')),
                     coalesce(max(de.visit_date), date('1000-01-01')),
                     coalesce(max(enr.visit_date), date('1000-01-01'))) as Date)      as LastVisit,
       ...
       COALESCE(c.key_population_type, (select case mid(max(concat(f.visit_date, f.key_population_type)), 11)
                                                   WHEN 105 THEN 'PWID'
       ...
       If(d.date_last_modified is NULL or hiv.date_last_modified is NULL
           or prg.date_last_modified is NULL,NULL,
       GREATEST(ifnull(d.date_last_modified,'0000-00-00'), ifnull(hiv.date_last_modified,'0000-00-00'),
                ifnull(prg.date_last_modified,'0000-00-00')))as Date_Last_Modified,
...
from kenyaemr_etl.etl_hiv_enrollment hiv
     ...
     (
         ...
         left join kenyaemr_etl.etl_contact c
